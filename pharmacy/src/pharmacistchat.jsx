...
const MyChat = () => {
    ...
    const acceptChat = async ({ user, userId, roomId }) => {
        appendMessage('You joined', true);
        return new Promise((resolve, reject) => {
            socket.emit('accept-chat', { user, userId, roomId }, (acknowledgment) => {
                if (acknowledgment.success) {
                    resolve();
                } else {
                    reject(new Error('Failed to accept chat'));
                }
            });
        });
    };
    ...
    useEffect(() => {
        ...
        console.log(roomId)
        acceptChat({ user: userId, userId, roomId }).catch(error => {
            console.error('Failed to accept chat:', error.message);
        });
